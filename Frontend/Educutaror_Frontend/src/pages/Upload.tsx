    ...
    <div className="max-w-2xl space-y-4">
      ...
      >
        ...
        {!file ? (
          <>
            ...
            <p className="text-sm text-gray-400">PDF, DOCX o TXT · máx. 50 MB</p>
          </>
        ) : (
      ...
      {state === 'success' && (
        <div className="flex items-center gap-2 bg-green-50 border border-green-200 text-green-800 text-sm rounded-xl px-4 py-3">
          ...
          <span>¡Documento subido! El agente comenzará a procesarlo pronto.</span>
          <button
            onClick={reset}
      ...
      {state === 'error' && (
        <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl px-4 py-3">
          <AlertCircle className="w-4 h-4 shrink-0" />
          {errorMsg}
          <button onClick={reset} className="ml-auto text-red-700 hover:underline font-medium shrink-0">
            Intentar de nuevo
          </button>
        </div>
      )}
